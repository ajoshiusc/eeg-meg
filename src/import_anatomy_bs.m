...
% XMLFile = file_find(bst_fileparts(TessLhFile), 'brainsuite_labeldescription*.xml');
% if isempty(XMLFile)
%     errorMsg = [errorMsg 'Label description file was not found: brainsuite_labeldescription.xml.' 10 'If this file is not present directly in the segmentation folder,' 10 'please copy it manually there.'];
% end
...
if ~isempty(TessLwFile)
    ...
    % Load sphere
    if ~isempty(TessLsphFile)
        bst_progress('start', 'Import FreeSurfer folder', 'Loading registered sphere: left inner...');
...
if ~isempty(TessRwFile)
    ...
    % Load sphere
    if ~isempty(TessRsphFile)
        bst_progress('start', 'Import FreeSurfer folder', 'Loading registered sphere: right inner...');
