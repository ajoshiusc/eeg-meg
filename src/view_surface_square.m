function [hFig, iDS, iFig] = view_surface_square(SurfaceFile)
% VIEW_SURFACE_Square: Display the registration Square for a surface.
%
...
if ~isfield(TessMat, 'Reg') || ~isfield(TessMat.Reg, 'Square')  || ~isfield(TessMat.Reg.Square, 'Vertices') || isempty(TessMat.Reg.Square.Vertices)
    bst_error('There is no registered Square available for this surface.', 'View registered Square', 0);
    return;
end
...
% If there is a Structures atlas with left and right hemispheres: split in two Squares
if ~isempty(ir) && ~isempty(il) && ~isConnected
    sphVertices(il,2) = sphVertices(il,2) + .155;
    sphVertices(ir,2) = sphVertices(ir,2) - .155;
end
...
% Display Square + results
    ...
    GlobalData.Surface(iSurfNew).FileName = [GlobalData.Surface(iSurf).FileName, '|Squares'];
...
% Force sulci display
panel_surface('SetSurfaceSmooth', hFig, iTess, 0.2);
